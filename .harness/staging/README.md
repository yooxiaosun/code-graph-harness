...
> 正式目录 `.harness/extractors/` 只能通过晋级闸门 `scripts/promote-extractor.sh` 写入。
...
```
.harness/staging/sdk/<name>/     # 一个可复用能力 = 一个 SDK 包
├── <name>.sh                    # 函数库（可被多个提取器 source）
└── test-<name>.sh               # 测试样例（全绿才可请求晋级）
```
晋级通道：`bash scripts/promote-sdk.sh <name>` → `scripts/base/<name>.sh`

## 交付包准入要求

1. **三件套齐全**：`extract-<pattern>.sh` + `fixtures/sample-<pattern>/` + `fixtures/expected/<pattern>.json`，缺一不可
2. **脚本规范**：`#!/usr/bin/env bash` + `set -euo pipefail`；参数 `<service-name> <repo-path> [output-dir]`；通过 `ROOT_DIR="$(cd "$SCRIPT_DIR/../../.." && pwd)"` 定位工程根，source `$ROOT_DIR/scripts/base/java-parser.sh` 与 `$ROOT_DIR/scripts/base/json-writer.sh`（晋级后落位 `.harness/extractors/<pattern>/extract.sh`，相对层级一致）；输出 `nonstandard-<pattern>.json`
3. **自证全绿**：`bash scripts/e4-verify-bundle.sh <pattern>` 必须 exit 0（GP1 语法 / GP2 执行 / GP3 JSON / GP4 召回 / GP5 回归）
4. **模式命名**：小写 kebab-case（如 `redis-client`）；不得与既有提取器目录重名（见 `.harness/extractors/`）
...
- 任何人/AI 不得在 staging 外绕过晋级闸门直接修改 `.harness/extractors/**`
