...
# 7-phase execution: 0→Deps, 1→Repos, 2→Nodes (L1), 3→Edges (L2), 4→Calibrate (L3), 5→Assemble, 6→Summary
set -euo pipefail
...
echo "  Architecture: Nodes(L1) → Edges(L2) → Calibrate(L3) → Assemble"
echo ""
...
tool_phase() {
    case "$1" in
        ...
        jq)    echo "Phase 2-4 Nodes/Edges/Calibrate (JSON processing)" ;;
...
if [ -f "$REPO_CONFIG" ]; then
    source "$EXTRACTORS_DIR/base/repo-manager.sh"
...
if [ -f "$REPO_CONFIG" ] && [ "$(get_repo_count "$REPO_CONFIG")" -gt 0 ] 2>/dev/null; then
    while IFS= read -r repo_url; do
        ...
        bash "$EXTRACTORS_DIR/graph/build-nodes.sh" "$REPO_NAME" "$REPO_PATH" "$NODES_DIR"
    done < <(get_repo_urls "$REPO_CONFIG")
fi
echo ""
...
bash "$EXTRACTORS_DIR/graph/build-edges.sh" "$NODES_DIR" "$EDGES_DIR"
echo ""

# ─────────────────────────────────────────────────────────────────────
# Phase 4: Calibration (Layer 3)
# ─────────────────────────────────────────────────────────────────────
run_phase "4" "Calibration (Layer 3: 5 checks)"
if bash "$EXTRACTORS_DIR/graph/calibrate.sh" "$NODES_DIR" "$EDGES_DIR" "$CALIBRATION_DIR"; then
    echo ""
else
    echo "[ABORT] Calibration blocked — check output/calibration/calibration-report.json"
    exit 1
fi
echo ""
...
bash "$EXTRACTORS_DIR/graph/assemble-graph.sh" "$NODES_DIR" "$EDGES_DIR" "$CALIBRATION_DIR" "$GRAPH_DIR/latest.json"
echo ""
...
bash "$EXTRACTORS_DIR/graph/merge-graphs.sh" "$NODES_DIR" "$GRAPH_DIR"
echo ""
