...
# 检查 1: match_rate 达标
MATCH_RATE=$(json_getdef "$EDGE_STATS" match_rate 0)
...
# 检查 2: blocker 为空
BLOCKER_COUNT=$(python3 -c "import sys,json; print(len(json.load(open('$CALIBRATION')).get('blockers',[])))" 2>/dev/null || echo "-1")
if [ "$BLOCKER_COUNT" = "0" ]; then
    pass "blockers 为空"
elif [ "$BLOCKER_COUNT" = "-1" ]; then
    fail "calibration-report.json 中无 blockers 字段或 JSON 损坏"
else
    python3 -c "import sys,json; [print('    blocker:', b) for b in json.load(open('$CALIBRATION')).get('blockers',[])]" 2>/dev/null
    fail "blockers 非空 (count=$BLOCKER_COUNT) — 需升级 User 决策"
fi

# 检查 3: unresolved 数据可读
UNRESOLVED_COUNT=$(json_getdef "$EDGE_STATS" unresolved 0)
TOTAL_COUNT=$(json_getdef "$EDGE_STATS" total_consumers 0)
...
SCORE=$(json_getdef "$CALIBRATION" overallScore 0)
...
total = 0
for f in glob.glob('$svc_dir/*.json'):
    try:
        for node in json.load(open(f)):
            if isinstance(node, dict) and node.get('metadata', {}).get('dual_dimension_consistency') == 'contradiction':
                total += 1
    except Exception:
        pass
