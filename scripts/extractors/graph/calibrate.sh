...
# Layer 3: Calibration
# 5 checks on Layer 1+2 outputs → calibration report
...
echo "[CALIBRATE] Running 5 checks..."
BLOCKERS=()
WARNINGS=()
...
if [ -f "$EDGES_DIR/edge-stats.json" ]; then
    ...
    if command -v bc &>/dev/null; then
        if [ "$(echo "$E_SCORE >= 0.90" | bc 2>/dev/null)" = "1" ]; then
            E_RATING="GOOD"
        elif [ "$(echo "$E_SCORE >= 0.70" | bc 2>/dev/null)" = "1" ]; then
            E_RATING="FAIR"
        else
            E_RATING="POOR"
            WARNINGS+=("Completeness score is POOR (< 70%) — consider AI analysis for missing patterns")
        fi
    fi
fi
echo "    score: $E_SCORE, rating: $E_RATING"
...
cat > "$REPORT_FILE" <<REPORT
{
  ...
  "checks": {
    ...
    "E_completenessScore": {"score": $E_SCORE, "matched": $E_MATCHED, "total": $E_TOTAL, "rating": "$E_RATING"}
  },
...
echo ""
echo "[CALIBRATE] Report: $REPORT_FILE"
echo "  Score: $E_SCORE ($E_RATING)"
echo "  Blockers: ${BLOCKERS_COUNT:-${#BLOCKERS[@]}}"
echo "  Warnings: ${#WARNINGS[@]}"

if [ "${#BLOCKERS[@]}" -gt 0 ]; then
    echo "[CALIBRATE] BLOCKED — ${#BLOCKERS[@]} blocker(s) found"
    exit 1
fi

echo "[CALIBRATE] PASS — no blockers"
